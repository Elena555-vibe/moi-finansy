...
const normalizedCategories = (categories?: Category[]) => {
  ...
  return [...current, ...existing.filter((category) => !matched.has(category.id)).map((category) => ({ ...category, archived: true }))];
};
