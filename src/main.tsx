...
const normalizedCategories = (categories?: Category[]) => {
  ...
  return [...current, ...existing.filter((category) => !matched.has(category.id)).map((category) => ({ ...category, archived: category.icon === 'custom' ? false : true }))];
};
...
function App() {
  ...
  const calc = useMemo(() => {
    ...
    const balance = data.initialBalance + income + debtReceived - totalExpenses - debtPaid; const reserve = reserves - withdrawals; const limit = plan.plannedIncome - plan.plannedReserve;
    const legacyDebtInitial = data.debt?.initialAmount || 0;
    return { month, plan, income, totalExpenses, monthExpenses, balance, reserve, free: balance - reserve, limit, remaining: limit - monthExpenses, remainingReserve: plan.plannedReserve - reserve, remainingSavings: data.savings.targetAmount - reserve, debtReceived, debtPaid, debtOutstanding: legacyDebtInitial + debtReceived - debtPaid };
  }, [data]);
  ...
  return <main className="app">
    ...
    {page === 'plan' && <PlanPage data={data} calc={calc} onSave={(plan) => commit({ ...data, plans: [...data.plans.filter((p) => p.month !== calc.month), plan] })} onSavings={() => setSheet('savings')} onDebt={() => setSheet('debt')} />}
...
function Operations({data,onEdit,onDelete,onFutureActions,onDeleteFuture}:{data:Data;onEdit:(t:Transaction)=>void;onDelete:(id:string)=>void;onFutureActions:(p:FuturePayment)=>void;onDeleteFuture:(id:string)=>void}) { const items=[...data.transactions].sort((a,b)=>b.date.localeCompare(a.date)); return <section><header><h1>Операции</h1></header><div className="operation-summary"><span>Доходы <b>{money(items.filter(x=>x.type==='income').reduce((s,x)=>s+x.amount,0))}</b></span><span>Расходы <b>{money(items.filter(x=>x.type==='expense').reduce((s,x)=>s+x.amount,0))}</b></span></div>{items.length===0&&!data.future.length?<div className="empty">Операций пока нет. Нажмите «+», чтобы добавить первую.</div>:<div className="list">{items.map(t=>{const isPositive=t.type==='income'||t.type==='savings_withdrawal'||t.type==='debt_received';const title=t.type==='income'?(data.sources.find(s=>s.id===t.sourceId)?.name||'Доход'):t.type==='reserve'?'В копилку':t.type==='savings_withdrawal'?'Из копилки':t.type==='debt_received'?'Взяла в долг':t.type==='debt_payment'?'Вернула долг':data.categories.find(c=>c.id===t.categoryId)?.name||'Расход';return <article key={t.id} className="operation"><div><strong>{title}</strong><small>{dateLabel(t.date)}{t.comment ? <> · <span className="operation-comment">{t.comment}</span></> : ''}</small></div><b className={isPositive?'positive':''}>{isPositive?'+':'−'}{money(t.amount)}</b><button onClick={()=>onEdit(t)} aria-label="Изменить">•••</button><button onClick={()=>onDelete(t.id)} aria-label="Удалить"><Trash2 size={16}/></button></article>})}{data.future.map(p=><article key={p.id} className="operation future"><div><strong>{data.categories.find(c=>c.id===p.categoryId)?.name||'Платёж'}</strong><small>{dateLabel(p.date)} · <span className="future-status">{p.paid?'Оплачено':'Ожидается'}</span></small></div><b>{money(p.amount)}</b>{!p.paid&&<><button onClick={()=>onFutureActions(p)} aria-label="Действия с ожидаемым платежом"><MoreHorizontal size={20}/></button><button onClick={()=>onDeleteFuture(p.id)} aria-label="Удалить ожидаемый платёж"><Trash2 size={16}/></button></>}</article>)}</div>}</section> }

function PlanPage({data,calc,onSave,onSavings,onDebt}:{data:Data;calc:any;onSave:(p:Plan)=>void;onSavings:()=>void;onDebt:()=>void}) { const [income,setIncome]=useState(String(calc.plan.plannedIncome||'')); const [reserve,setReserve]=useState(String(calc.plan.plannedReserve||'')); return <section><header><h1>Копилка</h1></header><div className="hero"><p>Лимит на месяц</p><strong>{money((Number(income)||0)-(Number(reserve)||0))}</strong><div className="progress"><i style={{width:`${Math.max(0,Math.min(100, calc.monthExpenses / Math.max(1,(Number(income)||0)-(Number(reserve)||0))*100))}%`}}/></div></div><label>Планируемый доход<MoneyInput value={income} onChange={setIncome}/></label><label>Плановый резерв<MoneyInput value={reserve} onChange={setReserve}/></label><button className="primary" onClick={()=>onSave({month:calc.month,plannedIncome:Number(income)||0,plannedReserve:Number(reserve)||0})}>Сохранить план <Check size={18}/></button><div className="reserve-card savings-summary"><p>В копилке</p><strong>{money(calc.reserve)}</strong><small>{data.savings.targetAmount ? `До цели осталось: ${money(calc.remainingSavings)}` : 'Цель пока не задана'}</small></div><button className="secondary savings-manage-action" onClick={onSavings}>Управлять копилкой <ChevronRight size={18}/></button><div className="reserve-card debt-summary"><p>Долговые обязательства</p><strong>К возврату: {money(calc.debtOutstanding)}</strong><small>Возвращено: {money(calc.debtPaid)}</small></div><button className="secondary savings-manage-action" onClick={onDebt}>Управлять долговыми обязательствами <ChevronRight size={18}/></button></section> }

function SettingsPage({data,onSave,onCloud}:{data:Data;onSave:(d:Data)=>void;onCloud:()=>void}) { const [balance,setBalance]=useState(String(data.initialBalance)); const [categoryName,setCategoryName]=useState(''); const [sourceName,setSourceName]=useState(''); const addCategory=()=>{if(categoryName.trim()) {onSave({...data,categories:[...data.categories,{id:uid(),name:categoryName.trim(),icon:'custom'}]});setCategoryName('');}}; const addSource=()=>{if(sourceName.trim()) {onSave({...data,sources:[...data.sources,{id:uid(),name:sourceName.trim()}]});setSourceName('');}}; return <section><header><h1>Настройки</h1></header><label>Начальный остаток<MoneyInput value={balance} onChange={setBalance}/></label><button className="secondary" onClick={()=>onSave({...data,initialBalance:Number(balance)||0})}>Сохранить остаток</button><section className="cloud-card cloud-protection"><div><p>Защита данных</p><small>{data.cloud ? (data.cloud.dirty ? 'Есть несинхронизированные изменения' : `Сохранено: ${data.cloud.lastSyncedAt ? new Intl.DateTimeFormat('ru-RU',{dateStyle:'short',timeStyle:'short'}).format(new Date(data.cloud.lastSyncedAt)) : 'сервер подключён'}`) : 'Серверная копия пока не подключена'}</small></div><button onClick={onCloud}>{data.cloud ? 'Открыть' : 'Подключить'}</button></section><h2>Категории</h2><div className="settings-list">{data.categories.filter(c=>!c.archived).map(c=><div key={c.id}><CategoryIcon icon={c.icon}/><span>{c.name}</span><button onClick={()=>onSave({...data,categories:data.categories.map(x=>x.id===c.id?{...x,archived:true}:x)})}>Архивировать</button></div>)}</div><div className="inline-add"><input value={categoryName} onChange={e=>setCategoryName(e.target.value)} placeholder="Новая категория"/><button onClick={addCategory}>Добавить</button></div><h2>Источники дохода</h2><div className="settings-list">{data.sources.filter(s=>!s.archived).map(s=><div key={s.id}><span>{s.name}</span><button onClick={()=>onSave({...data,sources:data.sources.map(x=>x.id===s.id?{...x,archived:true}:x)})}>Архивировать</button></div>)}</div><div className="inline-add"><input value={sourceName} onChange={e=>setSourceName(e.target.value)} placeholder="Новый источник дохода"/><button onClick={addSource}>Добавить</button></div></section> }
